...
这是一个本地优先的传统术数学习、检索、排盘和验证评分工作台。当前版本以「六爻」作为第一条完整样板线，已经打通资料分层、规则卡、装卦辅助、案例槽位、准确度校准、GitHub CI 和 Netlify 上线；紫微斗数已建立第一版术语和盘式结构资料层，奇门遁甲、大小六壬已预留独立体系入口，后续按同一套轻量架构扩展。
...
- [紫微术语 JSON](data/ziwei_terms.json)
...
- `data/ziwei_terms.json` 作为紫微术语检索 API
